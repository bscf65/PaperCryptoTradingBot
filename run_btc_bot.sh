...
Default SCRIPT_FILE:
  btc_eth_sol_coinbase_paper_bot_v16.py

Examples:
  ./run_btc_bot.sh --help
  ./run_btc_bot.sh btc_eth_sol_coinbase_paper_bot_v16.py --help
  ./run_btc_bot.sh btc_eth_sol_coinbase_paper_bot_v16.py --config configs/aggressive_100.json --once
  ./run_btc_bot.sh btc_eth_sol_coinbase_paper_bot_v16.py --config configs/aggressive_100.json --reset
  ./run_btc_bot.sh analyze_bot_performance_v16.py

Notes:
  - This runner does not place trades.
  - The v16 bot uses Coinbase public market data only.
  - Default products: BTC-USD, ETH-USD, SOL-USD.
  - v16 supports JSON config files in ~/btc-bot/configs/.
...
SCRIPT_FILE="${1:-btc_eth_sol_coinbase_paper_bot_v16.py}"
if [[ $# -gt 0 ]]; then
    shift
fi
