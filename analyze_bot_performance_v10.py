#!/usr/bin/env python3
"""
Analyze BTC/ETH/SOL Paper Bot v10 CSV logs.

Reads:
- logs/paper_trades_v10.csv
- logs/paper_equity_log_v10.csv
- logs/paper_daily_pnl_v10.csv
...
def analyze(log_dir: Path) -> int:
    trades = read_csv(log_dir / "paper_trades_v10.csv")
    equity = read_csv(log_dir / "paper_equity_log_v10.csv")
    daily = read_csv(log_dir / "paper_daily_pnl_v10.csv")
    tax = read_csv(log_dir / "paper_tax_capital_gains_v10.csv")

    print("\nBTC/ETH/SOL PAPER BOT v10 PERFORMANCE REPORT")
    print("=" * 80)
    ...
    if not daily.empty:
        ...
        print("-" * 80)
        cols = [c for c in ["timestamp_utc", "date_utc", "current_equity", "daily_pl", "daily_pl_pct", "alpha_vs_equal_weight_usd"] if c in daily.columns]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Analyze BTC/ETH/SOL paper bot v10 performance logs.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--log-dir", type=Path, default=LOG_DIR, help="Folder containing v10 CSV log files.")
