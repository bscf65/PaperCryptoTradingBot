#!/usr/bin/env python3
"""
BTC/ETH/SOL Coinbase Public-Data Paper Trading Bot v10
...
STATE_FILE = LOG_DIR / "paper_state_v10.json"
TRADES_FILE = LOG_DIR / "paper_trades_v10.csv"
EQUITY_FILE = LOG_DIR / "paper_equity_log_v10.csv"
DAILY_FILE = LOG_DIR / "paper_daily_pnl_v10.csv"
TAX_FILE = LOG_DIR / "paper_tax_capital_gains_v10.csv"
RESEARCH_FILE = LOG_DIR / "paper_research_log_v10.csv"

USER_AGENT = "btc-eth-sol-paper-bot-v10/1.0"

ANSI_RED = "\033[31m"
ANSI_RESET = "\033[0m"


def red_terminal_text(text: str, enabled: bool = True) -> str:
    """Return red ANSI terminal text when color output is enabled."""
    if not enabled:
        return text
...
def maybe_roll_day(state: dict[str, Any], current_equity: float) -> None:
    today = today_utc()
    if state.get("current_day_utc") != today:
...
def daily_profit_locked(state: dict[str, Any], equity: float, args: argparse.Namespace) -> tuple[bool, str]:
    ...
    today = state.get("current_day_utc", today_utc())
...
def log_trade(trade: dict[str, Any]) -> None:
    header = [
        "timestamp_utc", "product_id", "asset", "lot_id", "side", "qty", "market_price", "exec_price",
...
def log_tax(tax_row: dict[str, Any]) -> None:
    header = [
        "tax_year", "asset", "product_id", "lot_id", "acquire_date_utc", "dispose_date_utc",
...
def log_equity(state: dict[str, Any], snapshots: dict[str, dict[str, Any]], equity: float, fee_rate: float = 0.006) -> None:
    ...
    append_csv(
        ...
        equity_row,
        [
            "timestamp_utc", "cash", "equity", "realized_pl_total", "realized_pl_today",
            ...
            "open_positions", "trading_halted", "halt_reason", "daily_profit_locked_date",
    ...
    append_csv(
        ...
        daily_row,
        [
            "timestamp_utc", "date_utc", "daily_start_equity", "current_equity",
...
def maybe_log_news(products: list[str]) -> None:
    ...
    try:
        ...
        append_csv(
            ...
            ["timestamp_utc", "source", "query", "article_count_sample", "sample_titles", "note"],
        )
...
def run_cycle(state: dict[str, Any], products: list[str], args: argparse.Namespace) -> None:
    ...
    print("\n" + "=" * 94)
    print(f"COINBASE PUBLIC-DATA PAPER BOT v10 - {now_iso()}")
    print("=" * 94)
    print(f"Fee model: {args.fee_model} | effective simulated taker fee: {fee_rate:.4%}")
    print(f"Risk ladder: {risk['reason']} | trade size now ${adjusted_trade_size:,.2f} | buy score threshold {effective_threshold:.1f}")
    if interest_added > 0:
        print(f"Idle cash yield accrued this cycle: ${interest_added:.4f}")

    # Kill switch check before new actions.
    halted, reason = check_kill_switch(state, equity, args)
    if halted:
        state["trading_halted"] = True
        state["halt_reason"] = reason

    # Force-close positions if kill switch just triggered.
    if state.get("trading_halted"):
        print(f"TRADING HALTED: {state.get('halt_reason', '')}")
        for product in products:
            pos = state["positions"][product]
            if safe_float(pos.get("qty", 0.0), 0.0) > 0:
                trade, tax = sell_position(product, state, snapshots[product], args, "forced_exit_kill_switch")
                if trade:
                    log_trade(trade)
                if tax:
                    log_tax(tax)
    ...
    for product, snap in snapshots.items():
        ...
        print(red_terminal_text(score_line, enabled=not args.no_color))
...
def parse_products(value: str) -> list[str]:
    products = [x.strip().upper() for x in value.split(",") if x.strip()]
    if not products:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Coinbase public-data BTC/ETH/SOL paper-trading simulator v10 with benchmarks, risk ladder, spread filter, daily profit lock, idle-cash yield, and estimated fee/tax-aware capital-gains CSV.",
    ...
    parser.add_argument("--max-drawdown-pct", type=float, default=0.10, help="Hard halt if total account drawdown reaches this percentage. 0 disables percentage drawdown halt.")
    ...
    parser.add_argument("--reset", action="store_true", help="Delete v10 simulation state/log files before starting a clean paper simulation.")
    ...
    parser.add_argument("--no-color", action="store_true", help="Disable ANSI terminal colors in the live display.")
    return parser
...
def main() -> int:
    ...
    if args.reset:
        reset_files()
        print("Reset complete: deleted v10 simulation state/log files.")

    products = args.products if isinstance(args.products, list) else DEFAULT_PRODUCTS
    ...
    print("Starting Coinbase public-data paper bot v10. Press Ctrl+C to stop.")
