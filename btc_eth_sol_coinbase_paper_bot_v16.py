#!/usr/bin/env python3
"""
BTC/ETH/SOL Coinbase Public-Data Paper Trading Bot v16
...
- Adds spread filters, drawdown risk ladder, daily profit lock, configurable fee presets, fee-aware trade filtering, best-only selection mode, and max-open-position controls.
...
STATE_FILE = LOG_DIR / "paper_state_v16.json"
TRADES_FILE = LOG_DIR / "paper_trades_v16.csv"
EQUITY_FILE = LOG_DIR / "paper_equity_log_v16.csv"
DAILY_FILE = LOG_DIR / "paper_daily_pnl_v16.csv"
TAX_FILE = LOG_DIR / "paper_tax_capital_gains_v16.csv"
RESEARCH_FILE = LOG_DIR / "paper_research_log_v16.csv"

USER_AGENT = "btc-eth-sol-paper-bot-v16/1.0"
...
def trade_tally_for_product(state: dict[str, Any], product_id: str) -> dict[str, int]:
    """Return and initialize the BUY/SELL tally for a product."""
    tallies = state.setdefault("trade_tallies", {})
    tally = tallies.setdefault(product_id, {"BUY": 0, "SELL": 0})
    tally.setdefault("BUY", 0)
    tally.setdefault("SELL", 0)
    return tally


def increment_trade_tally(state: dict[str, Any], product_id: str, side: str) -> dict[str, int]:
    """Increment the per-product trade tally after a simulated order."""
    tally = trade_tally_for_product(state, product_id)
    side = side.upper()
    if side in ("BUY", "SELL"):
...
def tally_text(state: dict[str, Any], product_id: str) -> str:
    """Readable per-coin tally for terminal and CSV snapshots."""
    tally = trade_tally_for_product(state, product_id)
    buys = int(tally.get("BUY", 0))
    sells = int(tally.get("SELL", 0))
    return f"BUY {buys} | SELL {sells} | TOTAL {buys + sells}"


def all_tallies_text(state: dict[str, Any], products: list[str] | None = None) -> str:
    """Readable summary of all per-coin tallies."""
    if products is None:
        products = list(state.get("trade_tallies", {}).keys())
    return " ; ".join(f"{product}:{tally_text(state, product)}" for product in products)


def total_trade_counts(state: dict[str, Any]) -> tuple[int, int]:
    """Return total BUY and SELL counts across all products."""
    total_buys = 0
    total_sells = 0
    for tally in state.get("trade_tallies", {}).values():
        total_buys += int(tally.get("BUY", 0))
        total_sells += int(tally.get("SELL", 0))
...
def score_trade_setup(s: dict[str, Any]) -> tuple[float, str]:
    ...
    if regime == "normal":
        ...
        # v16: low-volatility chop is dangerous when fee drag is high, so penalize it.
        score -= 8
...
def initial_state(starting_cash: float, products: list[str]) -> dict[str, Any]:
    return {
        ...
        "halted_since_utc": "",
        "resume_confirm_count": 0,
        "auto_resume_count_date_local": today_local(),
        "auto_resume_count_today": 0,
        "recovery_mode_active": False,
        "recovery_resume_time_utc": "",
        "recovery_original_halt_reason": "",
...
def load_state(starting_cash: float, products: list[str]) -> dict[str, Any]:
    ...
    for p in products:
        state["positions"][p].setdefault("entry_exec_price", 0.0)
    ...
    state.setdefault("halted_since_utc", "")
    state.setdefault("resume_confirm_count", 0)
    state.setdefault("auto_resume_count_date_local", today_local())
    state.setdefault("auto_resume_count_today", 0)
    state.setdefault("recovery_mode_active", False)
    state.setdefault("recovery_resume_time_utc", "")
    state.setdefault("recovery_original_halt_reason", "")
    ...
    state.setdefault("trade_tallies", {})
    for p in products:
        state["trade_tallies"].setdefault(p, {"BUY": 0, "SELL": 0})
        state["trade_tallies"][p].setdefault("BUY", 0)
        state["trade_tallies"][p].setdefault("SELL", 0)
    return state
...
def open_position_accounting(pos: dict[str, Any], snap: dict[str, Any], fee_rate: float) -> dict[str, float]:
    ...
    entry_exec_price = safe_float(pos.get("entry_exec_price", 0.0), safe_float(pos.get("avg_price", 0.0), 0.0))
...
def expected_move_pct_for_trade(snap: dict[str, Any], args: argparse.Namespace) -> float:
    """Estimate whether there is enough potential movement to overcome costs.

    This is intentionally simple and conservative. It is not a prediction; it is a
    sanity check that refuses tiny-chop trades where expected movement is smaller
    than the estimated round-trip fee/spread cost.
    """
    atr_pct = abs(safe_float(snap.get("atr_pct", 0.0), 0.0))
    positive_momentum = max(
        0.0,
        safe_float(snap.get("ret_5", 0.0), 0.0),
        safe_float(snap.get("ret_15", 0.0), 0.0),
        safe_float(snap.get("ret_30", 0.0), 0.0),
    )
    atr_projection = atr_pct * max(1.0, safe_float(getattr(args, "take_profit_atr_multiple", 4.0), 4.0))
    momentum_projection = positive_momentum * 2.0
    return max(atr_projection, momentum_projection)


def round_trip_cost_pct_for_trade(snap: dict[str, Any], args: argparse.Namespace) -> float:
    """Approximate round-trip trading friction as entry fee + exit fee + spread."""
    fee_rate = effective_fee_rate(args, "taker")
    spread = max(0.0, safe_float(snap.get("spread_pct", 0.0), 0.0))
    return (2.0 * fee_rate) + spread
...
def should_skip_new_buy(snap: dict[str, Any], state: dict[str, Any], equity: float, args: argparse.Namespace) -> tuple[bool, str]:
    ...
    expected_move = expected_move_pct_for_trade(snap, args)
    round_trip_cost = round_trip_cost_pct_for_trade(snap, args)
    min_net_edge = normalize_rate(getattr(args, "min_expected_net_edge_pct", 0.0))
    required_move = round_trip_cost + min_net_edge
    if expected_move < required_move:
        return True, (
            "fee-aware no-buy: expected_move "
            f"{expected_move:.4%} < round_trip_cost+min_edge {required_move:.4%} "
            f"(cost {round_trip_cost:.4%}, min_edge {min_net_edge:.4%})"
        )

    return False, ""


def max_open_positions_allowed(state: dict[str, Any], args: argparse.Namespace) -> int | None:
    """Return max open positions, or None for unlimited. Recovery mode has its own cap."""
    if state.get("recovery_mode_active"):
        return max(1, int(getattr(args, "resume_max_open_positions", 1)))
    value = int(getattr(args, "max_open_positions", 1))
    return None if value < 0 else max(0, value)


def selected_buy_products(
    products: list[str],
    ...
    """Select which products may open new positions this cycle.

    all_signals: no selection filter; every qualifying asset may buy until max_open_positions is reached.
    best_only: only the highest-scoring qualifying asset may buy.
    top_two: only the two highest-scoring qualifying assets may buy, subject to capacity.
    """
    mode = str(getattr(args, "selection_mode", "best_only")).lower()
    if mode == "all_signals":
        return None

    max_open = max_open_positions_allowed(state, args)
    capacity = len(products) if max_open is None else max(0, max_open - open_position_count(state))
    if capacity <= 0:
        return set()
    ...
    for product in products:
        ...
        skip, _ = should_skip_new_buy(snap, state, equity, args)
        if skip:
            continue
    ...
    if mode == "top_two":
        ...
        n = min(1, capacity)
    return {product for _, product in candidates[:n]}
...
def buy_position(product: str, state: dict[str, Any], snap: dict[str, Any], trade_size: float, fee_rate: float) -> dict[str, Any] | None:
    ...
    tally = increment_trade_tally(state, product, "BUY")
    ...
    return {
        ...
        "asset_buy_tally": int(tally.get("BUY", 0)),
        "asset_sell_tally": int(tally.get("SELL", 0)),
        "asset_total_tally": int(tally.get("BUY", 0)) + int(tally.get("SELL", 0)),
...
def sell_position(product: str, state: dict[str, Any], snap: dict[str, Any], args: argparse.Namespace, reason: str) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    ...
    tally = increment_trade_tally(state, product, "SELL")
    ...
    tax = {
        ...
        "buy_exec_price_usd": usd(pos.get("entry_exec_price", 0.0)),
...
def log_trade(trade: dict[str, Any]) -> None:
    header = [
        ...
        "fee_usd", "trade_cost_usd", "total_trade_cost_usd", "cash_after", "realized_pl_usd", "trade_score",
        "asset_buy_tally", "asset_sell_tally", "asset_total_tally", "reason",
    ]
...
def log_tax(tax_row: dict[str, Any]) -> None:
    header = [
        ...
        "buy_avg_price_usd", "buy_exec_price_usd", "sell_exec_price_usd",
...
def log_equity(state: dict[str, Any], snapshots: dict[str, dict[str, Any]], equity: float, fee_rate: float = 0.006) -> None:
    ...
    equity_row = {
        ...
        "halted_since_utc": state.get("halted_since_utc", ""),
        "resume_confirm_count": state.get("resume_confirm_count", 0),
        "auto_resume_count_today": state.get("auto_resume_count_today", 0),
        "recovery_mode_active": state.get("recovery_mode_active", False),
        "recovery_resume_time_utc": state.get("recovery_resume_time_utc", ""),
        "trade_tallies": all_tallies_text(state, list(snapshots.keys())),
        "total_buy_tally": total_trade_counts(state)[0],
        "total_sell_tally": total_trade_counts(state)[1],
    ...
    append_csv(
        ...
        [
            ...
            "halted_since_utc", "resume_confirm_count", "auto_resume_count_today", "recovery_mode_active", "recovery_resume_time_utc",
            "trade_tallies", "total_buy_tally", "total_sell_tally",
    ...
    append_csv(
        ...
        [
            ...
            "trade_tallies", "trading_halted", "halt_reason", "auto_resume_count_today", "recovery_mode_active", "total_buy_tally", "total_sell_tally",
            "benchmark_btc_equity", "benchmark_eth_equity", "benchmark_sol_equity", "benchmark_equal_weight_equity",
            "alpha_vs_btc_usd", "alpha_vs_eth_usd", "alpha_vs_sol_usd", "alpha_vs_equal_weight_usd",
        ],
    )
...
def should_sell(pos: dict[str, Any], snap: dict[str, Any], args: argparse.Namespace) -> tuple[bool, str]:
    ...
    entry_exec_price = safe_float(pos.get("entry_exec_price", 0.0), avg)
    qty = safe_float(pos.get("qty", 0.0), 0.0)
    if qty <= 0:
    ...
    # v16: stops/targets use the actual simulated execution price, not the
    # fee-inflated average price/cost basis. This avoids overly tight stops.
    stop_price = entry_exec_price - (args.stop_atr_multiple * atr)
    trailing_stop = highest - (args.trailing_atr_multiple * atr)
    take_profit_price = entry_exec_price + (args.take_profit_atr_multiple * atr)

    if price <= stop_price:
        return True, f"stop_loss_atr price {price:.2f} <= {stop_price:.2f}"

    if highest > entry_exec_price and price <= trailing_stop:
...
def mark_temporary_halt(state: dict[str, Any], reason: str) -> None:
    """Mark a yellow temporary halt and start the cooldown timer once."""
    already_halted = bool(state.get("trading_halted")) and not bool(state.get("absolute_stop_triggered"))
    reason_changed = state.get("halt_reason", "") != reason
    if not already_halted or reason_changed or not state.get("halted_since_utc"):
        state["halted_since_utc"] = now_iso()
        state["resume_confirm_count"] = 0
    state["trading_halted"] = True
    state["halt_reason"] = reason
...
def minutes_since_utc(value: str) -> float:
...
def open_position_count(state: dict[str, Any]) -> int:
    return sum(1 for pos in state.get("positions", {}).values() if safe_float(pos.get("qty", 0.0), 0.0) > 0)


def reset_auto_resume_daily_count_if_needed(state: dict[str, Any]) -> None:
    today = today_local()
    if state.get("auto_resume_count_date_local") != today:
        state["auto_resume_count_date_local"] = today
        state["auto_resume_count_today"] = 0


def resume_market_check(snapshots: dict[str, dict[str, Any]], args: argparse.Namespace) -> tuple[bool, str, str, float]:
    """Return whether the market is healthy enough to resume after a yellow halt."""
    max_spread = normalize_rate(getattr(args, "max_spread_pct", 0.006))
    resume_score = safe_float(getattr(args, "resume_score_threshold", 70.0), 70.0)
    best_product = ""
    best_score = -999.0
    best_reason = "no products checked"
    for product, snap in snapshots.items():
        score = safe_float(snap.get("trade_score", 0.0), 0.0)
        spread = safe_float(snap.get("spread_pct", 0.0), 0.0)
        regime = str(snap.get("volatility_regime", ""))
        trend_ok = safe_float(snap.get("price", 0.0), 0.0) > safe_float(snap.get("ema50", 0.0), 0.0) and safe_float(snap.get("ema20", 0.0), 0.0) > safe_float(snap.get("ema50", 0.0), 0.0)
        spread_ok = spread <= max_spread
        vol_ok = regime != "extreme" or bool(getattr(args, "allow_extreme_vol_trades", False))
        reason = f"{product}: score={score:.1f}/{resume_score:.1f}, trend_ok={trend_ok}, spread={spread:.4%}/{max_spread:.4%}, vol={regime}"
        if score > best_score:
            best_product, best_score, best_reason = product, score, reason
        if score >= resume_score and trend_ok and spread_ok and vol_ok:
            return True, reason, product, score
    return False, best_reason, best_product, best_score


def maybe_auto_resume_from_halt(state: dict[str, Any], snapshots: dict[str, dict[str, Any]], args: argparse.Namespace) -> tuple[bool, str]:
    """Resume from a yellow halt only after cooldown plus market-confirmation criteria."""
    if not bool(state.get("trading_halted")) or bool(state.get("absolute_stop_triggered")):
        return False, "not in resumable temporary halt"
    if not bool(getattr(args, "auto_resume_after_halt", True)):
        return False, "auto resume disabled"
    if not state.get("halted_since_utc"):
        state["halted_since_utc"] = now_iso()
    cooldown = max(0.0, safe_float(getattr(args, "halt_cooldown_minutes", 120.0), 120.0))
    elapsed = minutes_since_utc(state.get("halted_since_utc", ""))
    if elapsed < cooldown:
        state["resume_confirm_count"] = 0
        return False, f"cooldown active: {elapsed:.1f}m elapsed, {cooldown - elapsed:.1f}m remaining"
    reset_auto_resume_daily_count_if_needed(state)
    max_resumes = int(getattr(args, "max_auto_resumes_per_day", 1))
    if max_resumes >= 0 and int(state.get("auto_resume_count_today", 0)) >= max_resumes:
        return False, f"max auto-resumes reached for {state.get('auto_resume_count_date_local')}: {state.get('auto_resume_count_today')} >= {max_resumes}"
    if open_position_count(state) > 0:
        state["resume_confirm_count"] = 0
        return False, "waiting for open positions to close before auto-resume"
    ok, reason, product, score = resume_market_check(snapshots, args)
    required = max(1, int(getattr(args, "resume_confirm_cycles", 3)))
    if not ok:
        state["resume_confirm_count"] = 0
        return False, f"resume criteria not met: {reason}"
    state["resume_confirm_count"] = int(state.get("resume_confirm_count", 0)) + 1
    if int(state.get("resume_confirm_count", 0)) < required:
        return False, f"resume confirmation {state.get('resume_confirm_count')}/{required}: {reason}"
    original_reason = state.get("halt_reason", "")
    state["trading_halted"] = False
    state["halt_reason"] = ""
    state["halted_since_utc"] = ""
    state["resume_confirm_count"] = 0
    state["auto_resume_count_date_local"] = today_local()
    state["auto_resume_count_today"] = int(state.get("auto_resume_count_today", 0)) + 1
    state["recovery_mode_active"] = True
    state["recovery_resume_time_utc"] = now_iso()
    state["recovery_original_halt_reason"] = original_reason
    state["alerted_temp_halt_reason"] = ""
    return True, f"AUTO-RESUME: cooldown passed and {product} met criteria ({reason}). Recovery mode enabled."
...
def run_cycle(state: dict[str, Any], products: list[str], args: argparse.Namespace) -> None:
    ...
    if state.get("recovery_mode_active"):
        adjusted_trade_size = max(0.0, adjusted_trade_size * safe_float(getattr(args, "resume_trade_size_factor", 0.5), 0.5))
        effective_threshold += safe_float(getattr(args, "resume_threshold_boost", 20.0), 20.0)
    fee_rate = effective_fee_rate(args, "taker")
    benchmarks = benchmark_values(state, snapshots, equity)
    allowed_buy_products = selected_buy_products(products, snapshots, state, equity, args, effective_threshold)

    print("\n" + "=" * 94)
    print(f"COINBASE PUBLIC-DATA PAPER BOT v16 - Local {local_now_iso()} | UTC {now_iso()}")
    print("=" * 94)
    ...
    if state.get("recovery_mode_active"):
        risk_line += f" | RECOVERY MODE: size x{safe_float(getattr(args, 'resume_trade_size_factor', 0.5), 0.5):.2f}, threshold +{safe_float(getattr(args, 'resume_threshold_boost', 20.0), 20.0):.1f}"
    if risk.get("hard_halt") or str(risk.get("reason", "")).startswith("drawdown_ladder") or state.get("recovery_mode_active"):
        print(color_text(risk_line, "yellow", args))
    else:
        print(risk_line)
    selection_desc = str(getattr(args, "selection_mode", "best_only"))
    max_open = max_open_positions_allowed(state, args)
    max_open_text = "unlimited" if max_open is None else str(max_open)
    if allowed_buy_products is None:
        allowed_text = "all qualifying signals"
    else:
        allowed_text = ", ".join(sorted(allowed_buy_products)) if allowed_buy_products else "none this cycle"
    print(f"Selection: {selection_desc} | max open positions {max_open_text} | allowed new buys: {allowed_text}")
    if interest_added > 0:
    ...
    if not state.get("absolute_stop_triggered"):
        ...
        if halted:
            mark_temporary_halt(state, reason)

    # Force-close positions if a halt triggered. Yellow halts can auto-resume after cooldown + confirmation.
    if state.get("trading_halted"):
        if not state.get("absolute_stop_triggered"):
            resumed, resume_reason = maybe_auto_resume_from_halt(state, snapshots, args)
            if resumed:
                print(color_text(resume_reason, "yellow", args))
                terminal_beep(args, 1)
            else:
                print(color_text(f"AUTO-RESUME WAIT: {resume_reason}", "yellow", args))
    ...
    for product, snap in snapshots.items():
        ...
        if qty > 0:
            ...
            elif max_open_positions_allowed(state, args) is not None and open_position_count(state) >= max_open_positions_allowed(state, args):
                action = "NO_BUY"
                action_reason = f"max open positions reached ({open_position_count(state)} >= {max_open_positions_allowed(state, args)})"
            elif allowed_buy_products is not None and product not in allowed_buy_products:
                action = "WATCH"
                action_reason = f"selection_mode {getattr(args, 'selection_mode', 'best_only')} did not select this asset"
        ...
        print(f"  Cost check:      expected move {expected_move_pct_for_trade(snap, args):.4%} | round-trip cost {round_trip_cost_pct_for_trade(snap, args):.4%} | min net edge {normalize_rate(getattr(args, 'min_expected_net_edge_pct', 0.0)):.4%}")
        print(f"  Action:          {action} - {action_reason}")
        print(f"  Tally:           {tally_text(state, product)}")
        if qty_after > 0:
            print(f"  Lot:             {pos_after.get('lot_id', '')}")
            print(f"  Position:        {qty_after:.8f} {product_to_asset(product)} avg/cost ${safe_float(pos_after.get('avg_price'), 0.0):,.2f}")
            print(f"  Entry exec px:   ${acct['entry_exec_price_usd']:,.2f}")
    ...
    total_buys, total_sells = total_trade_counts(state)
    print(f"Trade tallies:         {all_tallies_text(state, products)}")
    print(f"Total buys/sells:      BUY {total_buys} | SELL {total_sells} | TOTAL {total_buys + total_sells}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Coinbase public-data BTC/ETH/SOL paper-trading simulator v16 with local-time CSV logging, JSON config-file support, benchmarks, auto-resume yellow halts, risk ladder, spread filter, daily profit lock, idle-cash yield, and estimated fee/tax-aware capital-gains CSV.",
    ...
    parser.add_argument("--selection-mode", choices=["all_signals", "best_only", "top_two"], default="best_only", help="Which qualifying assets may open new positions each cycle. best_only helps small accounts avoid over-allocation.")
    parser.add_argument("--max-open-positions", type=int, default=1, help="Maximum simultaneous open positions. Use -1 for unlimited. Recovery mode can impose its own lower cap.")
    parser.add_argument("--min-expected-net-edge-pct", type=float, default=0.002, help="Minimum expected move above estimated round-trip fees/spread before opening a trade. 0.002 = 0.20 percent. Set 0 to only require break-even cost coverage.")
    ...
    parser.add_argument("--auto-resume-after-halt", action=argparse.BooleanOptionalAction, default=True, help="Automatically resume after a yellow temporary halt once cooldown and market-quality criteria are met. Red absolute stops never auto-resume.")
    parser.add_argument("--halt-cooldown-minutes", type=float, default=120.0, help="Minutes to wait before the bot can auto-resume after a yellow halt. Default is 120 minutes / 2 hours.")
    parser.add_argument("--resume-score-threshold", type=float, default=70.0, help="Minimum best-coin score required before auto-resuming after a yellow halt.")
    parser.add_argument("--resume-confirm-cycles", type=int, default=3, help="Number of consecutive cycles the resume criteria must pass before auto-resume.")
    parser.add_argument("--resume-trade-size-factor", type=float, default=0.50, help="Trade-size multiplier while recovery mode is active after auto-resume.")
    parser.add_argument("--resume-threshold-boost", type=float, default=20.0, help="Extra buy-score threshold points while recovery mode is active after auto-resume.")
    parser.add_argument("--resume-max-open-positions", type=int, default=1, help="Maximum open positions allowed while recovery mode is active after auto-resume.")
    parser.add_argument("--max-auto-resumes-per-day", type=int, default=1, help="Maximum yellow-halt auto-resumes allowed per local day. Set -1 for unlimited in paper tests.")
    ...
    parser.add_argument("--reset", action="store_true", help="Delete v16 simulation state/log files before starting a clean paper simulation.")
...
def main() -> int:
    ...
    if args.reset:
        reset_files()
        print("Reset complete: deleted v16 simulation state/log files.")
    ...
    print("Starting Coinbase public-data paper bot v16. Press Ctrl+C to stop.")
